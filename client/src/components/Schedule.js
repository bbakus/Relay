...
export const Schedule = () => {
    ...
    const generateICalendar = async () => {
        try {
            // Fetch ALL events from the API (not just current day)
            const response = await fetch(`${API_CONFIG.baseUrl}/api/events`)
            if (!response.ok) {
            ...
            // Filter events based on photographer selection
            ...
            eventsToExport = eventsToExport.filter(event => event.date >= today)
            ...
            if (eventsToExport.length === 0) {
                alert('No upcoming events to export. Please select a photographer or ensure there are events scheduled.')
                return
            }
        ...
        // Create filename based on photographer
        const photographerName = selectedPhotographerId 
            ? personnel.find(p => p.id === parseInt(selectedPhotographerId))?.name.replace(/\s+/g, '_') || 'Photographer'
            : 'All_Events'
                        ...
                        <div className='schedule-filters'>
                            ...
                            <div className='filter-group'>
                                <button 
                                    ...
                                    title='Download schedule as iCalendar file (.ics) for Apple Calendar'
                                >
                                    <span>📅</span>
                                    Download iCal
                                </button>
